...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func, model):
    """
    ...
    subdivs = np.array([np.transpose(patch, (2, 0, 1)) * WINDOW_SPLINE_2D for patch in subdivs])
...
def predict_img_with_smooth_windowing(input_img, window_size, subdivisions, nb_classes, pred_func, model ):
    """
    ...
    for pad in tqdm(pads):
        # For every rotation:
        sd = _windowed_subdivs(pad, window_size, subdivisions, nb_classes, pred_func, model)
